...
function PairPresenceBar({ state, onNotify, onNavigate }: { state: HabiState | null; onNotify: (message: string) => void; onNavigate: (view: ViewName) => void }) {
  const partner = state?.partner;
  const partnerOnline = Boolean(state?.partnerOnline);
  const roomCode = state?.room.code ?? '';
  const partnerLastSeen = state?.members.find((member) => !member.isCurrentUser)?.lastSeenAt;
  const partnerStatus = partner ? (partnerOnline ? '现在在线' : partnerLastSeen ? `${formatRelativeTime(partnerLastSeen)}在线过` : '等待上线') : '等待加入';
  const copyInvite = async () => {
    const copied = await copyToClipboard(inviteUrl(roomCode));
    onNotify(copied ? '邀请链接已复制，发给你的伙伴即可加入。' : '当前设备暂时不能复制，请手动分享房间码。');
  };

  return <section className="pair-presence-bar surface-card" aria-labelledby="pair-presence-title"><div className="pair-presence-heading"><div><p className="section-kicker"><span className="kicker-line" /> 双人在线空间</p><h2 id="pair-presence-title">把彼此放进<em>同一天。</em></h2></div><span className={`presence-sync-label ${partnerOnline ? 'is-live' : ''}`}><span className="live-dot" /> {partnerOnline ? '实时同步中' : partner ? '等待对方上线' : '等待伙伴加入'}</span></div><div className="pair-presence-body"><div className="presence-people"><div className="presence-person"><Avatar initials={state?.currentUser.initials ?? '你'} tone="sage" size="regular" /><div><strong>{state?.currentUser.name ?? '你'}</strong><span><i className="presence-dot is-online" /> 你 · 在线</span></div></div><span className="presence-bridge" aria-hidden="true">↔</span><div className="presence-person"><div className={`presence-avatar-shell ${partnerOnline ? 'is-online' : ''}`}><Avatar initials={partner?.initials ?? '?'} tone={partner ? 'clay' : 'lavender'} size="regular" /><i className={`presence-dot ${partnerOnline ? 'is-online' : ''}`} /></div><div><strong>{partner?.name ?? '等待伙伴'}</strong><span><i className={`presence-dot ${partnerOnline ? 'is-online' : ''}`} /> {partnerStatus}</span></div></div></div><div className="pair-presence-actions"><span>{partner ? '完成状态会自动同步给对方' : '复制邀请链接，让伙伴加入这座花园'}</span><button className="presence-link-button" type="button" disabled={!roomCode} onClick={() => { void copyInvite(); }}><Icon name="link" size={14} /> 复制邀请链接</button><button className="presence-settings-button" type="button" onClick={() => onNavigate('pair')}>连接设置 <Icon name="arrow" size={14} /></button></div></div></section>;
}
...
export default function HabiTogether() {
  const [activeView, setActiveView] = useState<ViewName>(() => inviteCodeFromUrl() ? 'pair' : 'today');
